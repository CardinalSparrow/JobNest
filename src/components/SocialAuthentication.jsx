import { browserSessionPersistence, getAuth, getRedirectResult, GoogleAuthProvider, OAuthProvider, setPersistence, signInWithPopup, signInWithRedirect } from 'firebase/auth'
...
const SocialAuthentication = ({ formData, setFormData, requiredFields, isSocialSignIn, setIsSocialSignIn }) => {

    const [authProvider, setAuthProvider] = useState('')

    // Function to handle Google and Apple sign in
    const handleAuthSignIn = async () => {
        const provider = authProvider === 'google' ? new GoogleAuthProvider() : new OAuthProvider('apple.com')
    ...
    const checkForMissingFields = (data) => {
        const socialAuthFields = requiredFields.filter(field => 
  ...
  return (
    <div>
        <div className='flex justify-center sm:gap-12 gap-4'>
            <button className='flex p-2 border-2 text-sm sm:text-base' onClick={() => {
                setAuthProvider("google")
                handleAuthSignIn()
            }}><FaGoogle className='place-self-center mr-2 size-6' />Sign up with Google</button>
            <button className='flex p-2 border-2 text-sm sm:text-base' onClick={() => {
                setAuthProvider("apple")
                handleAuthSignIn()
            }}><FaApple className='place-self-center mr-2 size-6' />Sign up with Apple</button>
        </div>
