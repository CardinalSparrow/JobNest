...
const Experience = () => {
  ...
  const deleteExperience = () => {
    ...
    const updatedExperience = experience.filter((_, index) => index !== editIndex);
    setExperience(updatedExperience);
  ...
  return (
    <div className='bg-[#97EFAB99] p-5 my-4'>
        <h1 className='text-2xl font-medium text-primary mb-2'>Job Experience <span className='text-sm font-normal'>{experience.length} job history</span></h1>
                ...
                <div key={index}>
                    {edit && editIndex === index ? (
                        <div className='grid w-1/3 py-4 gap-3'>
                            ...
                            />
                            <textarea
                            onChange={(e) => setNewExperience({...newExperience, description: e.target.value})}>{newExperience.description}</textarea>
        ...
        {
          !add ? (<p className='px-8 my-2 cursor-pointer' onClick={() => setAdd(true)}>Add more</p>)
          : (<p className='px-8 my-2 cursor-pointer' onClick={() => setAdd(false)}>Show less</p>)
        }
        {
          add && (
              <div className='grid w-1/3 py-4 gap-3'>
                ...
                onChange={(e) => setNewExperience({...newExperience, description: e.target.value})}></textarea>
                <button className='bg-primary text-white px-4 py-1 rounded-md ml-2' onClick={addExperience}>Add</button>
              </div>
          )
        }
